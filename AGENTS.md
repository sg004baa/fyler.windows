...
- [x] **M7-1 未配線コードの配線** — long path変換(long_path::to_fs を全FS操作直前に配線)/ OneDriveプレースホルダ警告(確認ダイアログ表示)/ ディレクトリcase sensitivity実測(FILE_CASE_SENSITIVE_INFO、apply preflight分岐)を実処理へ配線し、BaselineTreeのID検索をO(1)化(M7-3の一部)。trashクレートの拡張形式パス受け入れはWindows実機要検証(recycle.rs)。Linuxで対象テスト98件・workspace clippy警告ゼロ、Windows GNUクロスターゲットでclippy pass。Windows実機動作確認は未実施
