...
- `^`(旧`-`。oil.nvim互換より好みを優先し2026-07-05に変更)を `fyler_parent` にremap → `EditorEvent::NavigateParent`
- ルート変更の手順(app層):
...
> **実装済み(2026-07-05)**。M8-1〜M8-6すべてLinuxゲート(fmt/test/clippy/クロスターゲット)pass、
> headless RPCスモーク4件pass(実nvim)。3セッション実装:
> (A) 展開非再帰化+SetModifiable+ダイアログキー操作 / (B) ファイル情報表示+gyパスコピー /
> (C) config.toml設定+:bブックマーク・recent.toml。
> Windows実機動作確認は未実施(GUI目視・クリップボード・%APPDATA%読み書き)。
